...
> **Note:** This implementation uses [Ollama](https://ollama.com) as the local LLM backend instead of OpenAI — no API key required. Make sure `ollama serve` is running in a separate terminal before querying.
